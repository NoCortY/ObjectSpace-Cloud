package cn.objectspace.componentcenter.util;
...
public class SftpUtil {
     ...
     * 上传
     *
     * @param remoteDir 要上传到sftp上哪一个文件夹  【绝对路径】,如:/files/test/
     * @param src       要上传的文件的位置         【绝对路径】,如: /var/abc.txt
     ...
     * 文件下载
     *
     * @param remoteDirOrRemoteFile 要下载的sftp上的文件 或 sftp上的文件夹 【绝对路径】
     * @param localDir              用于存放下载的文件的本地文件夹 【绝对路径】
     *                              注:可以不存在，会自动创建
     *                              注:可以不存在，会自动创建
     * @param recursiveDownload     当remoteDirOrRemoteFile指向文件夹时，此参数生效；
     *                              为false时,
     ...
     *
     * @param dirPath 文件夹 全路径名 ， 如: /var/data/excel/
     ...
     * 注:如果要创建的文件夹的长辈级目录不存在，那么会进行相应的创建
     *
     * @param dirPath 文件夹 全路径名 ， 如: /files/a/b/c/
